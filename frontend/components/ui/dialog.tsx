...
>(({ className, children, ...props }, ref) => {
  ...
  // 在对话框关闭时移除焦点，避免 aria-hidden 警告
  React.useEffect(() => {
    const handleAnimationEnd = (e: AnimationEvent) => {
      if (e.animationName.includes('zoom-out') && contentRef.current) {
        // 对话框关闭动画结束时，移除内部元素的焦点
        const focusedElement = contentRef.current.querySelector(':focus') as HTMLElement
        if (focusedElement) {
          focusedElement.blur()
        }
      }
